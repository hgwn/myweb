/*======================================
 * 内部终端绑定查询页面控制器 inSearchTunnelController
 * =======================================
 * */
tunnelmanagerModule.controller("inSearchTunnelController",
    ['$scope','$rootScope', '$http', '$state',  '$timeout', '$modal','$interval','uiGridGroupingConstants','tunnelmanagerService','i18nService',function ($scope,$rootScope, $http, $state, $timeout,$modal,$interval,uiGridGroupingConstants,tunnelmanagerService,i18nService) {
      ...
      $scope.getinterterminalgroupsnameDataForm = {};  //设置获取内部组数据对象
      $scope.getinterterminalgroupsnameDataForm.pagenum = "";
      $scope.getinterterminalgroupsnameDataForm.pagesize="";
      $scope.getinterterminalgroupsnameDataForm.flag="1";   //0所有 1绑定终端  2没有绑定组
      //ui-grid 分页汉化
...
//    		          $scope.gridApi.grid.refresh();
                        ...
                        if(!row.entity.nodeLoaded){
                          $interval(function(){
                            if(interterminalgroupsid==undefined) return;
                              var url =  tunnelmanagerService.getAllbyinterterminalUrl()+"?flag=2&interterminalgroupsid="+interterminalgroupsid;
                      ...
                      if(!row.entity.nodeLoaded){
                          $interval(function(){
                            if(exterterminalgroupsid==undefined) return;
                              var url =  tunnelmanagerService.getAllbyinterterminalUrl()+"?flag=3&exterterminalgroupsid="+exterterminalgroupsid;
                              tunnelmanagerService.httpGet(url,function(data){
                                console.log("flag33");
                                console.log(data);
                                  var resulte = angular.copy(data);
        ...
        //加载内部终端号关联内部组数据
        $scope.getInterData = function(){
            //var url = 'json/t_search.in.json?dt='+Math.random();
            //var url = tunnelmanagerService.getAllbyinterterminalUrl()+"?flag=1&pagenum=1&pagesize=10";
            var url = tunnelmanagerService.getAllbyinterterminalUrl();
           ...
           // tunnelmanagerService.httpGet(url,function(data){
              ...
              console.log("加载内部终端号查询所有关于终端号的关联信息88");
              console.log(data);
              var resulte = angular.copy(data);
      ...
      //获取内部组数据
      $scope.getinterterminalgroupsnameData = function(){
          var url = tunnelmanagerService.getQueryinterterminalgroupUrl();
          tunnelmanagerService.httpPost(url,$scope.getinterterminalgroupsnameDataForm,function(data){
            console.log(data);
            $scope.interterminalgroupsnameData = data.data.rows;
          });
      }
      //调用获取内部组数据
      $scope.getinterterminalgroupsnameData();
