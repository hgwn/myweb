	...
	$(document).ready(function(){
		$("#exportPdf").click(function(){
			...
			if("${page.totalCount}">0){
				var url = "${contextPath }/report/orgPaySumReportD1?format=pdf&organization.code=${siteCode}";
				url = url + url_params;
				window.open(url);
			}else{
		...
		$("#exportExcel").click(function(){
			setParamsWhenSubmit();
			if("${page.totalCount}">0){
				var url = "${contextPath }/report/orgPaySumReportD1?format=xls&organization.code=${siteCode}";
				url = url + url_params;
				window.open(url);
			}else{
				alertMsg.info("请先查询出数据再导出！");	
...
<dwz:paginationForm action="${contextPath }/report/orgPaySumD1" page="${page }">
...
<div class="pageContent">
	<div class="tabs">
		<div class="tabsContent re_tabsContent">
			<div >
				<form method="post" id="reportB2_form" action="${contextPath }/report/orgPaySumD1" onsubmit="return setParamsWhenSubmit() && navTabSearch(this);">
				<input name="isFirst" type="hidden" value="1"/>
					...
					<div class="searchBar repeat_searchBar">
						<div class="searchContent repeat_searchContent">
							<table class="h-table" style="width:90%;">
								...
								<input style="width:95%;" class="validate[required] required textInput" type="text" name="organization.name" id="d1_siteName" value="${siteName}"/>
	...
	<div class="panelBar">
		<ul class="toolBar">
			...
			<c:if test="${isFirst!=null}">
				<div layoutH="95" id="orgPaySumReportD1Content" class="unitBox" >
					<c:import url="/report/orgPaySumReportD1?pageNum=${page.pageNum }&numPerPage=${page.numPerPage }" charEncoding="UTF-8" />
				</div>
