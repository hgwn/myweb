...
<script type="text/javascript">
	var a2_url_params = "";
	var setParamsWhenSubmitA2 = function() {
		a2_url_params = "";
		var orgId_a2 = $("#orgId_a2").val(), category_a2 = $("#category_a2").val(),year_a2=$("#year_a2").val(),season_a2 = $("#season_a2").val(),month_a2 = $("#month_a2").val(),monthOrseason_a2 = $("#monthOrseason_a2").val();
		a2_url_params = a2_url_params+"&orgId_a2=" + orgId_a2 + "&category_a2=" + category_a2 + "&year_a2=" + year_a2 + "&season_a2=" + season_a2 + "&month_a2=" + month_a2 + "&monthOrseason_a2=" + monthOrseason_a2;
		//alert("orgId_a2:"+orgId_a2+"--------category_a2:"+category_a2+"------season_a2:"+season_a2+"-------month_a2:"+month_a2+"-------monthOrseason_a2:"+monthOrseason_a2+"-------year_a2:"+year_a2);
		//alert(a2_url_params);
		return true;
	}
	$(document).ready(function() {
		...
		//用于搜索后回先时间搜索条件
		var key_a2 = $("#monthOrseason_a2").val();
		if (key_a2 == 1) {
			$("#month_a2").show();
			$("#season_a2").hide();
		}
		...
		$("#a2_exportPdf").click(function() {
			setParamsWhenSubmitA2();
			...
			var text = $("#orgPosTerReportA2").text();
			if(text.indexOf("没有符合条件的信息")>=0){
				alertMsg.info("请查询出数据再导出");
				return false;
			}
			
			var url = "${contextPath }/report/orgPosPayReportA2?format=pdf";
			url = url + a2_url_params;
			window.open(url);
		});
		$("#a2_exportExcel").click(function() {
			setParamsWhenSubmitA2();
			...
			var url = "${contextPath }/report/orgPosPayReportA2?format=xls";
			url = url + a2_url_params;
			window.open(url);
		});
		$("#monthOrseason_a2").click(function() {
			var key_a2 = $("#monthOrseason_a2").val();
			if (key_a2 == 1) {
				$("#month_a2").show();
				$("#season_a2").hide();
...
<div class="pageContent">
	<div class="tabs">
		<div class="tabsContent re_tabsContent">
			<div>
				<form method="post" action="${contextPath }/report/orgPosPayIndexA2"
					class="required-validate pageForm"
					onsubmit="return setParamsWhenSubmitA2() && navTabSearch(this);">
					<input type="hidden"  name="isFirst_a2" value="1" />
					<div class="pageHeader re_pageHeader">
						<div class="searchBar repeat_searchBar">
							...
							<table class="h-table" style="width:90%;">
								<input type="hidden" id="isFrist_a2" value="1" name="isFrist_a2" />
								<tr>
								<td width="90px" align="right">选择机构：</td>
								<td> <select id="orgId_a2" name="orgId_a2" style="width:98%;">
								...
								<td> <select id="category_a2"
									name="category_a2" style="width:98%;">
								...
								</select> <select id="monthOrseason_a2" name="monthOrseason_a2">
								...
								</select> <select id="month_a2" name="month_a2">
								...
								</select> <select id="season_a2" name="season_a2" style="display:none;">
				...
				<div class="panelBar">
					<ul class="toolBar">		   
							<li><a iconClass="report" id="a2_exportPdf"><span>导出PDF</span>
							</a>
							</li>
							<li><a iconClass="page_white_excel" id="a2_exportExcel"><span>导出Excel</span>
							</a>
							</li>
					</ul>
				</div>
					...
					<c:if test="${isFirst==null||isFisrt=='' }">
						<div layoutH="90" id="orgPostTerA2Content" class="unitBox">
							<br>	
					...
					<c:if test="${isFirst!=null}">
						<div layoutH="80" id="orgPosTerReportA2" class="unitBox" >
							<c:import url="/report/orgPosPayReportA2" charEncoding="UTF-8" />
						</div>
					</c:if>	
			</div>
		</div>
