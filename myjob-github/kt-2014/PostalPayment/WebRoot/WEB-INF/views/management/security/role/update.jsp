	      ...
	      for(Permission p : module.getPermissions()){
	        ...
	        for(RolePermission rolePermission : role.getRolePermissions()){
	            if(rolePermission.getPermission().getId().equals(p.getId())){
	                //builder.append("{id:\"p_"+p.getId()+"\", pId:\"m_"+module.getId()+"\",name:\""+p.getPermissionName().getKeyItem()+"\",permissionId:"+p.getId()+",permission_data:\""+permissionData+"\",isSelect :\""+rolePermission.getAccessScope()+"\" },");
	            	iSelectedValue = rolePermission.getAccessScope();
	            	break;
	            }
	        }
	        builder.append("{id:\"p_"+p.getId()+"\", pId:\"m_"+module.getId()+"\",name:\""+p.getPermissionName().getKeyItem()+"\",permissionId:"+p.getId()+",permission_data:\""+permissionData+"\",isSelect :\""+iSelectedValue+"\" },");
	      }
	    }
...
<%
	...
	String uPermissonTree = role_tree(module2, role2);
...
<form method="post" action="${contextPath }/management/security/role/update" class="required-validate pageForm" onsubmit="return validateCallback(this, navTabReloadParent);">
	...
	<dl class="h-dl">
		<dt>角色名称：</dt>
		<dd>
			<input type="text" name="name" value="${role.name }" class="input-medium validate[required,maxSize[64]] required" data-prompt-position="bottomRight:-55,-10" maxlength="64"  style="width:190px;"/>
		</dd>
	</dl>
	<dl class="h-dl">
		<dt>级别：</dt>
		<dd>
			<select name="levels" class="re_select196 validate[required] required">
				<c:forEach items="${roleLevels }" var="item">
					<option value="${item.item1 }" <c:if test="${item.item1==role.levels }">selected</c:if>>${item.keyItem }</option>
				</c:forEach>
			</select>
		</dd>
	</dl>
	<dl class="h-dl">
		...
		<dd>
			...
			<textarea name="description" rows="4" cols="24" class="w190" style="resize:none;" maxlength="256">${role.description }</textarea>
		</dd>
	</dl>
	</div>
		...
		<legend>自定义互斥角色</legend>
			<div class="h-set-box">
				<c:forEach var="item" items="${roles}" varStatus="status">
					...
					<label class="h-row-item h-ell"><input name="mutexRole" value="${item.id}" type="checkbox" <c:if test="${ischeck==1 }">checked</c:if>>${item.name}</label>
				</c:forEach>
			</div>		
		</fieldset>
	</dl>
	...
	<div style="border:1px solid #ddd; border-top:none;"><ul id="permissonTree" class="ztree"></ul></div>
	<script type="text/javascript">
	  $(function(){
		  HTree_select("#permissonTree_1_ul");
	  });
	</script>
	</div>
