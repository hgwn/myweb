...
function initLayout(){
	...
	$("#container .tabsPageContent").height(iContentH - 41).find("[layoutH]").layoutH();
...
function initUI(_box){
	...
	//自己添加
	if ($.fn.selectedOne) $("a[target=selectedOne]", $p).selectedOne();
	if ($.fn.selectedTab) $("a[target=selectedTab]", $p).selectedTab();
	if ($.fn.selectedAjax) $("a[target=selectedAjax]", $p).selectedAjax();
