...
var navTab = {
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		...
		if (iOpenIndex >= 0){
			...
			$tab.find(">a").attr("title", op.title).find(span$).html("<i class='"+op.data.icon_className+"'></i>"+op.title);
			...
			var tabFrag = '<li tabid="#tabid#"#pTabid#><a href="javascript:" title="#title#" class="#tabid#"><span><i class="'+op.data.icon_className+'"></i>#title#</span></a><a href="javascript:;" class="close">close</a></li>';
			// 添加pTabid属性，方便子navTab拿到父navTab的id
			if (op.data.pTabid) {
