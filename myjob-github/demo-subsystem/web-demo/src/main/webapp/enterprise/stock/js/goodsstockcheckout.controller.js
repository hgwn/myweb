var changewarehouseModule= angular.module('changewarehouseout',[]);

changewarehouseModule.controller('goodsstockcheckoutController',['$scope','$http','Upload',function($scope,$http,Upload){
	//定义数据对象
    $scope.addFormData={};
    $scope.addFormData.goodsstockcheckoutlist = [];
    ...
    $scope.addrecord=function(){
        ...
        temp.goodsqty=0;
        temp.damageqty=0;
        $scope.addFormData.goodsstockcheckoutlist.push(temp);
    };
    
    //删除一行移库单数据
    $scope.delHouseAddRecord=function(index){
        $scope.addFormData.goodsstockcheckoutlist.splice(index, 1);
    };
    ...
    $scope.showaddmodal=function(){
    	if($scope.addFormData.warehouseoutnum ==""||$scope.addFormData.warehouseoutnum==undefined){
    		 var today = new Date();
             ...
             $scope.addFormData.warehouseoutnum= "CK"+year +  month  + day +  hours +   minutes +  seconds;
    	}
    ...
    $scope.showaddmodal();
     ...
     $scope.OutgridOptions = { 
    		 ...
    		 columnDefs : [
    		                ...
    		                /*{ name: '状态',field:'resultImport',enableCellEdit: false, width: '15%' ,cellTemplate:
 			                   '<div class="ui-grid-cell-contents">{{grid.appScope.getState(row.entity.resultImport)}}</div>'
 			               	}*/
    		               ]
     };
     
     $scope.OutgridOptions.data = top.transferDataOut;
     //$scope.gridOptions.enableCellEditOnFocus = true;
    
     /*$scope.gridOptions.columnDefs = [
       { name: 'id', enableCellEdit: false },
       { name: 'goodsname', enableCellEditOnFocus:false, displayName:'商品名称', width: 200  },
       { name: 'skuid', enableCellEdit: true, displayName:'商品SKU', width: 300 },
       { name: 'skuid', enableCellEdit: true, displayName:'商品SKU', width: 300 },
       { name: 'purchaseprice', displayName: '进货价', width: 200}
     ];*/
    
     /*$http.get('/jsbweb/enterprise/com/get_goodsstock.do')
       .success(function(data) {
         $scope.OutgridOptions.data = data.data.rows;
       });*/
       ...
       $scope.OutgridOptions.onRegisterApi = function(gridApi){
       ...
       $scope.deleteGoods = function(row){
    	   console.warn(row);
       	angular.forEach($scope.OutgridOptions.data, function(value, index){
       		if(row === value)
       			$scope.OutgridOptions.data.splice(index, 1);
       	});
       };
       
       //新增一行
       $scope.addGoods = function(grid,myRow){
    	   var index = $scope.OutgridOptions.data.indexOf(myRow.entity)+1;
    	   var temp={};
           temp.goodsname="";
           temp.barcode="";
           temp.skuid="";
           ...
           $scope.OutgridOptions.data.splice(index,0,temp)
       }
       ...
       $scope.onClickImportAddGoods = function(){
       	...
       	console.warn(result);
	    	 $.jBox.confirm("确定要提交选中的"+result.length+"个商品吗？", "温馨提示", function(v, h, f){
	    		if(v == "ok"){
	    			$scope.addFormData.warehouseoutlist = result;
                    var data = { data: angular.toJson($scope.addFormData) };
	   	    		console.info(data);
		   	    		$http({
	                         method : 'POST',
	                         url : "/jsbweb/enterprise/com/add_stockout.do",
	                         data : $.param(data),
	                         headers : {
	                             'Content-Type' : 'application/x-www-form-urlencoded'
	                         }
