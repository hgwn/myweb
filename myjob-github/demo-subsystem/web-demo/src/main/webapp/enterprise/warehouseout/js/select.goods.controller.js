...
function($state, $scope, $rootScope, $http, $alert, $timeout, i18nService, masgetwebUtilService){
		...
		$scope.goodsGridOption = {
		      ...
		      { name: '库存数', width:'10%',field: 'inventoryqty' }
		      ],
...
//        $scope.getPage(1,$scope.goodsGridOption.paginationPageSize,null);
        ...
        var getPage = function(pagenum, pagesize, sort) {
            ...
            masgetwebUtilService.httpGet("/jsbweb/enterprise/stock/warehouseout/getStockGoods.do"
	        		...
	        		if(data.ret == 0){
	        			...
	        			angular.forEach(data.data.rows, function(goods){
	        				rows.push({
	    						...
	    						goodsunitprice:goods.purchaseprice,
	    						issernum:goods.issernum,
	    						isbatchcode:goods.isbatchcode,
	    						goodsstockid:goods.goodsstockid,
	    						inventoryqty:goods.goodqty,
	    						batchcode:''
	        				});
	        			});
