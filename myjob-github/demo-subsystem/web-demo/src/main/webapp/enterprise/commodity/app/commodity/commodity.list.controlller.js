...
controller('commodityController',['$scope','$rootScope','$cacheFactory','$http','$cookieStore','commodityService','$alert','$timeout','$state',
function($scope,$rootScope,$cacheFactory,$http,$cookieStore,commodityService,$alert,$timeout,$state){
    ...
    $rootScope.infoQueryDataFlag = function(){
    	$scope.queryFormData.busy = $rootScope.queryFormData.busy;
    };
    //分页查询
	$scope.commoditiesSearch = function(oders,orderKey,flag){
		$rootScope.queryFormData.busy = $scope.queryFormData.busy;
		$rootScope.commoditiesSearch(oders,orderKey,flag);
	};
    var arr ;
    //添加购物车、立即购买按钮触发的方法
	...
	$scope.state_go_detail = function (merchant,commodity) {
        $state.go("detail",{path:'123'});
        $rootScope.rootM = merchant;
        $rootScope.rootC = commodity;
    };
}]);