...
controller('headQuartersOrdersBillingController',['$state','$scope','$rootScope','$http','commodityService','regularValidationService','$alert','$timeout','headQuartersOrdersService',
function($state,$scope,$rootScope,$http,commodityService,regularValidationService,$alert,$timeout,headQuartersOrdersService){
	...
	$scope.submitOrders = function(){
        ...
        $scope.ordersParams.buyername = $scope.receiveData.addressItem.contactname;
        $scope.ordersParams.buyerphone = $scope.receiveData.addressItem.mobile;
        $scope.ordersParams.buyerprovinceid = $scope.receiveData.addressItem.provinceid;
        $scope.ordersParams.buyercityid = $scope.receiveData.addressItem.cityid;
        $scope.ordersParams.buyerareaid = $scope.receiveData.addressItem.areaid;
        $scope.ordersParams.buyeraddress = $scope.receiveData.addressItem.address;
        ...
        angular.forEach($scope.goodscart,function(data,index,array){
        	...
        	$scope.goodscart[index].giftflag = 1;  //赠品标志 1-不是
        	$scope.goodscart[index].goodsmoney = $scope.goodscart[index].dealingprice.toFixed(2) * $scope.goodscart[index].goodsqty;
        	$scope.goodscart[index].goodsmoney = $scope.goodscart[index].goodsmoney.toFixed(2);
        });
        $scope.ordersParams.orderlist = $scope.goodscart;
        ...
        $http({
        	...
        	if(data.ret == 0){
        		$alert({title: '提示：', content: '生成订货单成功!', placement: 'masget-top',type: 'info', duration:1, show: true});
        		headQuartersOrdersService.setPayData($scope.ordersParams);
        		console.info($scope.ordersParams.ordernum);
        		$timeout(function(){
        	    	$state.go("headQuartersOrdersPayment");
        	    },1000);
        	}else{
        		$alert({title: '提示：', content: '添加订货单失败,'+data.message+"!", placement: 'masget-top',type: 'info', duration:1, show: true});
        	}
        });
	};	

}])
.controller('headQuartersOrdersPaymentController',['$state','$scope','$rootScope','$http','headQuartersOrdersService','regularValidationService','$alert','$timeout',
function($state,$scope,$rootScope,$http,headQuartersOrdersService,regularValidationService,$alert,$timeout){
	console.info(headQuartersOrdersService.getPayData());
	console.info("1001");
	$scope.paramsData = {};
	$scope.paramsData = headQuartersOrdersService.getPayData();
		...
		if($scope.paramsData.paymentType == 1) {
			...
			$.ajax({
				...
				success:function(result) {
					...
					params.paystate = 3;
					headQuartersOrdersService.httpPost(headQuartersOrdersService.paidOrders(),params,function(resp){
						console.info(resp);
						if(resp.ret == 0){
...
controller('headQuartersOrdersAddressManageController',['utils','commodityService','$state','$scope','$rootScope','$http','headQuartersOrdersService','regularValidationService','$alert','$timeout',
function(utils,commodityService,$state,$scope,$rootScope,$http,headQuartersOrdersService,regularValidationService,$alert,$timeout){
	...
	$scope.saveAddressManage = function(){
		...
		if($scope.addressModifyAddFlag == 'add'){
			...
			console.log(params);
			headQuartersOrdersService.httpPost(headQuartersOrdersService.addAddressManage(),params,function(resp){
				console.info(resp);
			...
			params.remark = $scope.receiveData.remark;
			console.log(params);
			headQuartersOrdersService.httpPost(headQuartersOrdersService.modifyAddressManage(),params,function(resp){
				console.info(resp);
				$scope.getAddressmanage();
			});
		}
	...
	$scope.getAddressmanage = function(){
		...
		headQuartersOrdersService.httpPost(headQuartersOrdersService.getAddressManage(),params,function(resp){
			if(resp.ret == 0){
	...
	$scope.deleteAddress = function(receiveItem){
		...
		headQuartersOrdersService.httpPost(headQuartersOrdersService.deleteAddressManage(),params,function(resp){
			if(resp.ret == 0){
