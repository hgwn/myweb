...
controller('headQuartersOrdersAddController',['$state','$scope','$rootScope','$http','headQuartersOrdersService','regularValidationService','$alert',
function($state,$scope,$rootScope,$http,headQuartersOrdersService,regularValidationService,$alert){
	//查询参数
    ...
    $scope.searchGoodsList = function(){
    	...
    	$scope.goodsList_queryData.onlineflag = 1; //上架商品
    	headQuartersOrdersService.httpPost(headQuartersOrdersService.getGoodsList(),$scope.goodsList_queryData,function(resp){
    		if(resp.ret == 0){
...
.controller('goodsListController',['$state','$scope','$rootScope','$http','headQuartersOrdersService','regularValidationService','$alert',
function($state,$scope,$rootScope,$http,headQuartersOrdersService,regularValidationService,$alert){
	//验证购买数量输入是否合法
    ...
    $scope.addCart = function(item){
		...
		console.log($scope.goodsCart);	
    };	
}])
.controller('purchaseGoodsModalController',['$timeout','$state','$scope','$rootScope','$http','headQuartersOrdersService','$alert',
function($timeout,$state,$scope,$rootScope,$http,headQuartersOrdersService,$alert){
    ...
    $scope.billingGoodsCarts = function(){
    	if(ary.length==0){
			...
			$('#viewPurchaseGoods').modal('hide');
            $timeout(function(){
            	$state.go("headQuartersOrdersBilling");
            },500);
