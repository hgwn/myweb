...
//    		console.info($scope.formData.goodsid);
	...
	$scope.gridOption = {
          ...
          { name: '单号',field: 'ordernum' ,enableHiding: false,enableCellEdit: false , enableSorting: false, enableColumnResizing:true, width:'15%',
	    	  cellTemplate: '<div class="ui-grid-cell-contents" style="text-align:center;"><a href="#" class="" ng-click="grid.appScope.viewGoodsStockInOutDetail(grid,row.entity)" title="查看">{{row.entity.ordernum}}</a>&nbsp;'},
...
//            	console.info(data);
        ...
        //查看盘点明细
        $scope.viewGoodsStockInOutDetail = function(grid,entity){
        	$scope.detailQueryParams = {};
            $scope.detailQueryParams.pagenum = 1;
            $scope.detailQueryParams.pagesize = 100;
            $scope.detailQueryParams.ordernum = entity.ordernum;
            $scope.detailQueryParams.orderid = entity.orderid;
            
            if(entity.type==1)
            {
            	//调用入库的精确查询接口
                goodsstockService.httpPost(goodsstockService.getGoodsStockInList(),$scope.detailQueryParams,function(data){
                	if(data.ret == 0){
                		$scope.viewData = data.data.rows[0];
                		$("#viewGoodsStockInDetailId").modal("show");
                	}else if(data.ret == 10){
                    	$alert({title: '提示：', content: '请重新登录!', placement: 'masget-top',type: 'warning', duration:4, show: true});
                    }else{
                		$alert({title: '提示：', content: '查询商品入库历史记录异常,'+data.message, placement: 'masget-top',type: 'warning', duration:4, show: true});
                	}
                });
            } 
            else
            {
            	//调用出库的精确查询接口
                goodsstockService.httpPost(goodsstockService.getGoodsStockOutList(),$scope.detailQueryParams,function(data){
                	if(data.ret == 0){
                		$scope.viewData = data.data.rows[0];
//                		console.info($scope.viewData);
                		$("#viewGoodsStockOutDetailId").modal("show");
                	}else if(data.ret == 10){
                    	$alert({title: '提示：', content: '请重新登录!', placement: 'masget-top',type: 'warning', duration:4, show: true});
                    }else{
                		$alert({title: '提示：', content: '查询商品出库历史记录异常,'+data.message, placement: 'masget-top',type: 'warning', duration:4, show: true});
                	}
                });
