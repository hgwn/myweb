angular.module('masgetWebApp.workflow').controller('workflowEditCtr', ['$scope', '$state', '$timeout', '$dropdown', 'utils', 'businessTypes', function ($scope, $state, $timeout, $dropdown, utils, businessTypes) {
        ...
        if($state.broadCast&&$state.broadCast.operationType === 'edit'){
            ...
            $.each($scope.processNodes,function(key,item){
                ...
                utils.query('/jsbweb/workflow.do?type=getUsers&users='+JSON.stringify(users)).then(function(resp){
        ...
        $scope.saveOrUpdate = function () {
            ...
            utils.query('/jsbweb/workflow.do?type=save', {type: 'post',method:'post', data: {data: JSON.stringify(data)}}).then(function (resp) {
                $alert($scope.operateType == 'insert'?"成功创建流程":'成功更新流程');
                $state.go('home.workflow.list');
            }, function (resp) {
        ...
        $scope.$watch('processInfo.business_id', function (newVal, oldVal) {
            utils.query("/jsbweb/workflow.do?type=processnodelsel&businessid=" + newVal).then(function (resp) {
                $scope.processNodes = [
        ...
        $scope.addNodeUsers = function () {
            ...
            $modal({callback: function (element, msg) {
                ...
                contentTemplate: 'base/workflow/tpls/userSelectGrid.tpl.html',
        ...
        $scope.$on('selectUsers.show', function (modal) {
            stationDropDown = $dropdown($("#stationSelect"), {
                ...
                hoverHold:true,
                delay:200,
                template: "base/workflow/tpls/stationList.tpl.html",
